<x-layouts.auth :title="'Dashboard Admin Libris'">
    <div class="w-full space-y-8">
        <div class="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-white/8 p-8 backdrop-blur-xl md:flex-row md:items-center md:justify-between">
            <div class="space-y-2">
                <p class="text-sm uppercase tracking-[0.22em] text-amber-200">Dashboard Admin</p>
                <h1 class="text-4xl text-stone-50">Halo, {{ auth()->user()->name }}</h1>
                <p class="max-w-2xl text-stone-300">
                    Fase 4 aktif. Otentikasi session dan pemisahan peran admin sudah berjalan. Fase berikutnya akan mengisi area ini dengan modul buku, anggota, dan peminjaman.
                </p>
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-3xl border border-white/10 bg-stone-900/70 p-6">
                ...
                <p class="mt-3 text-3xl text-stone-50">Admin</p>
            </div>
            <div class="rounded-3xl border border-white/10 bg-stone-900/70 p-6">
                <p class="text-sm uppercase tracking-[0.18em] text-stone-400">Email</p>
                <p class="mt-3 text-xl text-stone-50">{{ auth()->user()->email }}</p>
            </div>
            <div class="rounded-3xl border border-white/10 bg-stone-900/70 p-6">
                <p class="text-sm uppercase tracking-[0.18em] text-stone-400">Akses</p>
                <p class="mt-3 text-xl text-stone-50">Area Admin Saja</p>
            </div>
        </div>
    </div>
</x-layouts.auth>
