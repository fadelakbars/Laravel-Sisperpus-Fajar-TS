            ...
            <x-ui.card padding="p-5">
                <div class="flex flex-col h-full justify-between gap-y-3">
                    <div class="flex items-center gap-x-3">
                        ...
                        <p class="text-sm font-semibold text-slate-700">Surat Bebas Pus</p>
                    </div>
                    
                    @if(auth()->user()->bisaDownloadSuratBebas())
                        <x-ui.button type="link" href="{{ route('anggota.surat-bebas') }}" target="_blank" variant="primary" size="sm" class="w-full text-xs py-2">
                            Download Surat
                        </x-ui.button>
                    @else
                        <div class="group relative">
                            <x-ui.button variant="secondary" size="sm" class="w-full text-xs py-2 opacity-50 cursor-not-allowed" disabled>
                                Download Surat
                            </x-ui.button>
                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 p-2 bg-slate-800 text-white text-[10px] rounded shadow-lg z-10 text-center">
                                Selesaikan pinjaman & denda untuk mengunduh surat.
                            </div>
                        </div>
                    @endif
                </div>
